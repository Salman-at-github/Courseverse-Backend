...
app.get('/api/student/:id', async (req, res) => {
    try {
        const studentID = parseInt(req.params.id); // Convert the id parameter to an integer
        ...
        if (student) {
            // Send the found student as the response
            res.status(200).json(student);
        } else {
            // If the student with the specified ID is not found, send a 404 response
